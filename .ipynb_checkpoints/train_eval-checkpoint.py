...
class Args:
    ...
    gpu: int
    time_embedding: str
    static_repr_dims: int
    dynamic_repr_dims: int
    epochs: int
    task_weights: dict
    run_name: str = ""
    batch_size: int = 16 
...
def train_hdst(train_data, device, config, run_dir, verbose, args):
    """
...
def train_and_test(args, verbose=1, save_eval_res=False):
    ...
    model = train_hdst(train_data, device, config, run_dir, verbose, args)
