...
class HDST:
    ...
    def fit0(self, train_data, k, w=None,top_k=3, temperature=1, n_epochs=None, n_iters=None):
        ''' Training the HDST model.
        Randomly choose k windows of size scale(batch-wise). The results are not stable.
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                for i in range(top_k):
                    ...
                    B, _, scale_new, nf=x_windows.shape
                    x_windows_reshaped = x_windows.reshape(B * k, scale_new, nf)  
                    ...
                    out_static = out_static.reshape(B, k, scale_new, out_dim1)
                    out_dynamic = out_dynamic.reshape(B, k, scale_new, out_dim2)
                    ...
                    out_shuffled_static = out_shuffled_static.reshape(B, k, scale_new, out_dim1)
    ...
    def fit(self, train_all, n_channels, distance='mcc', w=None,top_k=5, temperature=1, n_epochs=None, n_iters=None):
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                # print(multi_scale_loss.requires_grad)
                # multi_scale_loss.backward()
                # optimizer.step()
                # self.net.update_parameters(self._net)
    ...
    def fit2(self, train_data, k, w=None,top_k=2, temperature=1, n_epochs=None, n_iters=None):
        ''' Training the HDST model.
        Reshaped the input batch into shape (B, k, scale, C), where k*scale == n_timestampes.
        Using single latent feature. Use static and dynamic loss to train one feature.
        Args:
            train_data (numpy.ndarray): The training data. It should have a shape of (n_instance, n_timestamps, n_features). All missing data should be set to NaN.
            k (int) : Number of windows that randomly cropped while training.
            w (int) : Number of time point of each window (window size). Using multi-scale if not specified.
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                for i in range(top_k):
                    ...
                    # original windows
                    out_static, _=self._net(x_windows_reshaped) # (batch_size*k, scale, out_dims)
                    out_dim1 = out_static.size(-1)
                    # out_dim2 = out_dynamic.size(-1)
                    out_static = out_static.reshape(B, k, scale, out_dim1)
    ...
    def _eval_with_pooling2(
        ...
        '''
        Using sigle feature (no dynamic feature)
        '''
        out_static, _ = self.net(x.to(self.device, non_blocking=True),mask)
        out = out_static
    ...
    def encode2(
        ...
        ''' Compute representations using the model. Using single feature (no dynamic feature)
